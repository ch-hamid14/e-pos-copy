...
class ProductService {
  ...
  async create(
    ...
    return withTransaction(async (transaction) => {
      const [row] = await getDb()('products')
  ...
  async update(
    ...
    return withTransaction(async (transaction) => {
      const row = await getDb()('products').where({ id }).whereNull('deleted_at').first()
      if (!row) throw new Error('Product not found')

      const [updated] = await getDb()('products')
        .transacting(transaction)
        .where({ id })
        .update(withAuditUpdate(ctx, {
          ...
          ...(data.defaultPurchasePrice !== undefined && { default_purchase_price: data.defaultPurchasePrice }),
  ...
  async get(id: string): Promise<unknown> {
    const row = await getDb()('products as p')
      .leftJoin('categories as c', 'p.category_id', 'c.id')
      .where({ 'p.id': id })
      .whereNull('p.deleted_at')
      .select('p.*', 'c.name as category_name')
